...
module.exports = {
  ...
  getGraph: async (req, res) => {
    try {
      ...
      const conditionToolHistory = `WHERE tool_no = '${tool_no}' AND system_activity IN ('REGRINDING', 'SETTING')`;
      ...
      // Filter history_id berdasarkan system_activity
      const regrindingAndSettingHistories = toolHistories.filter(
        (history) =>
          history.system_activity === "REGRINDING" ||
          history.system_activity === "SETTING"
      );

      if (regrindingAndSettingHistories.length === 0) {
        return res.status(404).json({
          message:
            "No regrinding or setting data found for the given tool number",
        });
      }

      // Langkah 2: Ambil data dari tb_r_tool_checks berdasarkan setiap history_id
      const toolChecks = [];
      for (const history of regrindingAndSettingHistories) {
