...
module.exports = {
  ...
  getMeasuringGraph: async (req, res) => {
    try {
      ...
      const settingHistories = toolHistories.filter(
        (history) => history.system_activity === "SETTING"
      );

      if (settingHistories.length === 0) {
        return res.status(404).json({
          message: "No setting data found for the given tool number",
        });
      }
  ...
  getSettingGraph: async (req, res) => {
    try {
      ...
      if (parameter === "day") {
        ...
        const today = moment().format("YYYY-MM-DD");
        whereCond = `WHERE system_activity = 'SETTING' AND DATE(created_dt) = '${today}'`;
      } else if (parameter === "month") {
        const startOfMonth = moment().startOf("month").format("YYYY-MM-DD");
        const endOfMonth = moment().endOf("month").format("YYYY-MM-DD");
        whereCond = `WHERE system_activity = 'SETTING' AND DATE(created_dt) BETWEEN '${startOfMonth}' AND '${endOfMonth}'`;
      } else {
