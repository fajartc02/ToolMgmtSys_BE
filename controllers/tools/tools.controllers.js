  ...
  getTools: async (req, res) => {
    try {
      ...
      if (meta) {
        ...
        let query = `
          SELECT 
            ROW_NUMBER() OVER (ORDER BY tb_r_tools.created_dt DESC) as no,
            tb_r_tools.*, 
            tb_m_tool_types.tool_type_desc
          FROM 
            tb_r_tools
          INNER JOIN 
            tb_m_tool_types 
          ON 
            tb_r_tools.tool_type_id = tb_m_tool_types.tool_type_id
          WHERE 
            tb_r_tools.deleted_dt IS NULL ${
              conditions ? `AND ${conditions}` : ""
            }
          ORDER BY 
            tb_r_tools.created_dt DESC
        `;
        let result = await queryGET(query); // Menggunakan queryGET untuk query yang lebih kompleks
        success(res, "Success", result);
      }
    } catch (err) {
      console.log(err);
