...
module.exports = {
  ...
  querySoftDELETE: async (table, data, whereCond = "") => {
    return new Promise(async (resolve, reject) => {
      if (!whereCond) {
        return reject(new Error("Where condition is required for soft delete"));
      }
      ...
      try {
        ...
        // Loop melalui data untuk membangun query dan parameter terikat
        for (const key in data) {
          containerSetValues.push(`${key} = $${i}`);
          values.push(data[key]);
          i++;
        }

        // Query UPDATE dengan parameter terikat
        let q = `UPDATE ${table} SET ${containerSetValues.join(
          ", "
        )} WHERE ${whereCond}`;
        console.log(q);

        const result = await databasePool.query(q, values);
        resolve(result);
      } catch (err) {
        reject(err);
      }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (const key in data) {
        if (data[key] == "CURRENT_TIMESTAMP") {
          ...
          containerSetValues.push(`${key} = NULL`);
        }
      }
